...
export const FrameForm = () => {
  const { game, currentFrame, updatePlayers, nextFrame, prevFrame } =
  ...
  const updatePlayerScore = (player) => {
    ...
    if (currentFrame === 10) {
      ...
      newTotal =
        prevTotal + player.scores.reduce((sum, score) => sum + score.total, 0);
    }
    ...
    <form onSubmit={onSubmit} style={{ width: "600px" }}>
          ...
          <div className="inline" key={`frame-player-${index}`}>
            ...
            {currentFrame === 10 &&
              (frameScores[player.name]?.ball1 === 10 ||
                frameScores[player.name]?.ball1 +
                  frameScores[player.name]?.ball2 ===
                  10) && (
                <input
                  type="number"
      ...
      <div className="form-control">
        <div className="inline">
          ...
          <button className="btn" onClick={submitAndNextFrame}>
